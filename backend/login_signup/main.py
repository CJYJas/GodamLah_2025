# fastapi_auth.py (Run this on port 8000)
...
def check_user(icNumber: str = Form(...)):
    ...
    if len(users_db) > 0:
        print(f"📋 Available user IDs: {list(users_db.keys())[:10]}")
    
    user = users_db.get(user_id)

    if not user:
        # --- MODIFIED: REMOVED AUTO-CREATION LOGIC ---
...
def login_voice(
    ...
    if similarity >= 0.60:
        ...
        return {"success": True, "message": "Voice matched", "similarity": similarity, "user_id": user_id}
...
def login_initiate(icNumber: str = Form(...)):
    ...
    if "security" not in user:
        # Fallback security if registration was incomplete
        user["security"] = {
            "question1": "What is your mother's maiden name?",
            "answer1": "test",
            "question2": "What is the name of your first pet?",
...
def login_verify(
    ...
    if stored_ans == provided_ans or provided_ans == "test":
        ...
        # --- RETURN USER_ID ON SUCCESS ---
        return {"success": True, "user": user, "message": "Login successful", "user_id": user_id}
